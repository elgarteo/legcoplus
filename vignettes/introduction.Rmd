...
* [`all_members()`](#all_members) searches everyone who speaks in LegCo with unique identifiers
...
House Committee, Finance Committe and its subcommittees meetings on any given date, by any given committee/subcommittee or during any given meeting slot.
...
`SpeakerID` used in the Hansard Database and `MemberID` used in the Schedule Database. `all_members()` 
...
Note that if `verbose` is enabled (by default), multiple messages will be shown. The actual
number of records retrieved is only displayed in the last message. In the example above, it is 161.
